...
func Producer(userID, goodID uint) (err error) {
	message := fmt.Sprintf("%d:%d", userID, goodID)
	err = global.Redis.RPush(context.Background(), messageQueueKey, message).Err()
	if err != nil {
		global.Log.Printf("用户 %d 发起商品 %d 的秒杀请求时，加入队列失败: %v", userID, goodID, err)
		return
	}
	global.Log.Printf("用户 %d 发起商品 %d 的秒杀请求，已加入队列", userID, goodID)
	return nil
}
